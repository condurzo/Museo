...
public class ByteReaderCustom
{
	byte[] mBuffer;
	int mOffset = 0;

	public ByteReaderCustom (byte[] bytes) { mBuffer = bytes; }
	public ByteReaderCustom (TextAsset asset) { mBuffer = asset.bytes; }
	...
	public Dictionary<string, Dictionary<string,string>> ReadDictionary ()
	{
		...
		while (canRead)
		{
			...
			if (split.Length == 2)
			{
				...
				string[] interSplit = split[1].Split(intSeparator, System.StringSplitOptions.RemoveEmptyEntries);
				string intKey;
				
				if(interSplit.Length < 5){				
					for (int i = 0; i < interSplit.Length; i++) {
						if(i == 0){
							intKey = "xPos";
							internalDict[intKey] = interSplit[i];
						}else if(i == 1){
							intKey = "yPos";
							internalDict[intKey] = interSplit[i];
						}else if(i == 2){
